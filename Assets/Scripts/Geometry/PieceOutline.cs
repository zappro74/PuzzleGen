...
public static List<Vector2> BuildPieceOutline(PieceData piece, float pieceWidth, float pieceHeight, PieceConfig config)
    {
        ...
        List<Vector2> bottomEdge = BezierUtility.GenerateEdgeCurve(bottomRight, bottomLeft, piece.BottomEdge, Vector2.down, config.tabHeight, config.edgeMargin, config.tabWidth, config.pointsPerCurveHalf);
